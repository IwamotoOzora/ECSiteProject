...
public class RegisterItemServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (itemCode.isBlank() || name.isBlank() || price == 0 || color.isBlank() || category.isBlank()
			...
			RequestDispatcher rD = request.getRequestDispatcher("/views/administration_register.jsp");
			...
			messageBean.setSmsg("上記の内容で商品を登録しますか");
			request.setAttribute("messageBean", messageBean);

			request.setAttribute("itemBean", itemBean);

			RequestDispatcher rD = request.getRequestDispatcher("/views/register_check.jsp");
			rD.forward(request, response);
		}
	}
}
